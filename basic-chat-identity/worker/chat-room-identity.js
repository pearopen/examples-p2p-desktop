...
export default class ChatRoomIdentity extends ReadyResource {
  ...
  async _open () {
    ...
    this.pairMember = this.pairing.addMember({
      ...
      onadd: async (request) => {
        const inv = await this.view.findOne('@basic-chat-identity/invites', { id: request.inviteId })
        if (!inv) return
  ...
  _setupRouter () {
    this.router.add('@basic-chat-identity/add-writer', async (data, context) => {
      await context.base.addWriter(data.key)
    })
    this.router.add('@basic-chat-identity/add-invite', async (data, context) => {
      await context.view.insert('@basic-chat-identity/invites', data)
    })
    this.router.add('@basic-chat-identity/add-message', async (data, context) => {
      await context.view.insert('@basic-chat-identity/messages', data)
    })
  }
  ...
  async getInvite () {
    const existing = await this.view.findOne('@basic-chat-identity/invites', {})
    if (existing) {
      return z32.encode(existing.invite)
    }
    const { id, invite, publicKey, expires } = BlindPairing.createInvite(this.base.key)
    await this.base.append(
      ChatDispatch.encode('@basic-chat-identity/add-invite', { id, invite, publicKey, expires })
    )
  ...
  async addWriter (key) {
    await this.base.append(
      ChatDispatch.encode('@basic-chat-identity/add-writer', { key: b4a.isBuffer(key) ? key : b4a.from(key) })
    )
  }
